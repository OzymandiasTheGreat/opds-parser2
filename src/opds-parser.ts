...
function getValue(obj: object, subkey?: string) {
	if (!subkey) {
		subkey = "#";
	}
	if (obj && obj[subkey]) {
...
function toISOString(string: string, callback: Function) {
	try {
...
function handleMeta(node: object) {
	if (!node) {
		return {};
	}
	...
	Object.keys(node).forEach(function(name) {
		...
		switch (name) {
			...
			case "updated":
				...
				break;
			case "author":
				const getAuthor = function(el: object, callback: Function) {
					const author = {};
...
function handleEntry(node: object) {
	if (!node) {
		return {};
	}
	...
	Object.keys(node).forEach(function(name: string) {
		const el = node[name];
		...
		switch (name) {
			...
			case "title":
				...
				break;
			case "updated":
				toISOString(getValue(el), function(date: string) {
					entry.updated = date;
				});
				break;
			case "link":
			...
			case "content":
				...
				break;
			case "category":
				const getCategory = function(el: object, callback: Function) {
					const category = {};
			...
			case "published":
				...
				break;
			case "contributor":
				const getContributor = function(el: object, callback: Function) {
					const contributor = {};
...
class OpdsParser extends events.EventEmitter {
	...
	private handleOpenTag(node, scope) {
		const self = scope;
		const n = {
			"#name": node.name,
			"@": {},
		...
		function handleAttributes(attrs, el) {
			Object.keys(attrs).forEach(function(name) {
				if (self.xmlbase.length && (name === "href" || name === "src" || name === "uri")) {
					attrs[name] = new url(attrs[name], self.xmlbase[0]["#"]).toString(undefined);
	...
	private handleCloseTag(el, scope) {
		const self = scope;
	...
	private handleSaxError(e, scope) {
		const self = scope;
		self.handleError(e, self);
	}

	private handleError(e, scope) {
		const self = scope;
	...
	private _setCallback(callback) {
		this.callback = ("function" === typeof callback) ? callback : undefined;
	}

	public parse(feed: string, callback) {
		this._setCallback(callback);

		this.parser.write(feed).close();
	}

	public getFeedType(json, callback) {
		const self = this;
...
export function parseString(feed: string): Promise<Feed> {
	return new Promise((resolve, reject) => parser.parse(feed, (err, parsed) => err ? reject(err) : resolve(parsed)));
}

export function getFeedType(json: Feed): Promise<"acquisition" | "navigation" | null> {
